  ...
  const handleGenerateTryOn = async () => {
    ...
    try {
      ...
      try {
        ...
        // Step 2: Generate the image using IDM-VTON (Preserves identity and accurately maps clothing)
        console.log("Connecting to IDM-VTON model...");
        const app = await client("yisol/IDM-VTON");
        
        // Convert base64 to Blobs for Gradio
        const userBlob = await (await fetch(userPhoto)).blob();
        const clothingBlob = await (await fetch(clothingPhoto)).blob();
        
        console.log("Sending images to AI for processing (this may take 30-60 seconds)...");
        const result = await app.predict("/tryon", [
          { "background": userBlob, "layers": [], "composite": userBlob }, // Human image + auto-mask
          clothingBlob, // Garment image
          prompt, // Prompt generated by Gemini for context
        ...
        if (result.data && result.data[0]) {
          ...
          throw new Error("The AI model returned an empty result. Please try again.");
        }
