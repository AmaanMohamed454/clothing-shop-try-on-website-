...
function App() {
  ...
  const handleGenerateTryOn = async () => {
    ...
    try {
      ...
      try {
        console.log("Attempting high-precision try-on with IDM-VTON...");
        ...
        if (result.data && result.data[0]) {
          ...
          throw new Error("Empty result from IDM-VTON");
        }
      } catch (vtonErr) {
        console.warn("IDM-VTON failed or is busy. Falling back to robust generation...", vtonErr);
        // Fallback Step: Use Pollinations for robustness
        // We use the descriptive prompt generated by Gemini to recreate the look
        const encodedPrompt = encodeURIComponent(prompt + ", high quality, photorealistic, 8k, fashion photography");
        ...
        // Test if the image loads (optional but good for UX)
        console.log("Using Pollinations fallback...");
        setResultImage(fallbackUrl);
        // Warning: Pollinations might not perfectly preserve identity, but it will always generate an image.
      }

    } catch (err) {
      setError(`We encountered a temporary issue with the AI service. Please try again in a moment or use a different photo.`);
